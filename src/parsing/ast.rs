        ...
        match self {
            ...
            ValType::Cons(c) => {
                let p = c
                    .car()
                    .get_podedure_name()
                    .unwrap_or_else(|| panic!("Cant eval procedure{:?}", c))
                    .to_string();
                ...
                scheme_funcs::FUNCTIONS
                    .get(&p)
                    .unwrap_or_else(|| panic!("Procedure not bound"))(&v)
                .unwrap_or_else(|e| panic!("error evaluating procedure: {}", e))
            }
